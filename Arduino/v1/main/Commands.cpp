...
// Coupling obbligatorio: M4 segue sempre M2 in verso opposto
static const int   COUPLE_M4_SIGN   = -1;

// fail-safe: se non riceviamo pacchetti per X ms, stop ai delta
static const unsigned long RF_TIMEOUT_MS = 500;

// ========= STATO VELOCITÀ =========
static volatile int16_t v1 = 0, v2 = 0, v3 = 0; // [-100..100]
...
// target4 e' sempre derivato da target2 (asse virtuale accoppiato).
...
void joystickVelocityUpdate() {
  ...
  // timeout radio ⇒ vel 0
  if (now - lastRfMs > RF_TIMEOUT_MS) v1 = v2 = v3 = 0;
...
void processToken(const String &cmd)
{
  ...
  if (cmd == F("HOMING") || cmd == F("HOME")) {
    ...
    // Se ci sono dati seriali pendenti (es. config inviate dal PC durante
    // l'operazione) li scartiamo: l'homing è bloccante e verranno processati
    // dopo la fine dell'homing causando log "setConfiguration" imprevisti.
...
void setConfiguration(const String &config)
{
  ...
  if (sepIndex == -1)
  {
    Serial.println(F("Errore: formato parametro non valido"));
    return;
  }
  ...
  {
    Serial.println("Parametro sconosciuto: " + param);
  }
